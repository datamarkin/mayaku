...
def _solver_cfg(**overrides: object) -> SolverConfig:
    """Tiny SolverConfig for optimizer-construction tests."""
    base: dict[str, object] = {"num_epochs": 12}
...
# ---------------------------------------------------------------------------
# build_lr_scheduler — warmup → cosine (the only scheduler)
# ---------------------------------------------------------------------------


def _make_opt_and_cfg(**overrides: object) -> tuple[torch.optim.SGD, SolverConfig]:
    model = nn.Linear(2, 2)
    cfg_kwargs: dict[str, object] = {"warmup_factor": 0.001, "base_lr": 1.0}
...
def test_warmup_grows_linearly_from_factor_to_one() -> None:
    opt, cfg = _make_opt_and_cfg()
    # max_iter / warmup_iters are resolved at train time, so they're passed in.
    sched = build_lr_scheduler(opt, cfg, max_iter=1000, warmup_iters=100)
...
def test_warmup_cosine_decays_from_one_to_zero() -> None:
    opt, cfg = _make_opt_and_cfg()
    sched = build_lr_scheduler(opt, cfg, max_iter=110, warmup_iters=10)
    for _ in range(10):
        sched.step()
